...
CONFIG="ider_tinyimagenet_buf500_paper"
GPU="${1:-0}"
SEEDS="${2:-0 1 2 3 4}"
...
LOG_DIR="${PROJECT_ROOT}/output/runs/tinyimagenet_buf500_paper_${RUN_TAG}"
...
if [ ! -f "datasets/TINYIMG/processed/x_train_01.npy" ] || [ ! -f "datasets/TINYIMG/processed/y_val_20.npy" ]; then
  ...
  echo "Put the processed TinyImageNet files under datasets/TINYIMG/processed first."
  exit 1
fi

echo "Experiment : IDER, TinyImageNet, buffer=500, paper-style CIL"
echo "Config     : ${CONFIG}"
echo "GPU        : ${GPU}"
echo "Seeds      : ${SEEDS}"
